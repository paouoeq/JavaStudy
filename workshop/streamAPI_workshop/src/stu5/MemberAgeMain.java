...
public class MemberAgeMain {
	...
	public static void main(String[] args) {
		...
		int sum = members.stream().mapToInt(t->t.getAge()).sum();
		System.out.println("Sum : "+sum);
		double avg = members.stream().mapToDouble(t->t.getAge()).average().orElse(0.0);
		System.out.println("Avg : "+avg);
		int min = members.stream().mapToInt(t->t.getAge()).min().orElse(0);
		System.out.println("Min : "+min);
		int max = members.stream().mapToInt(t->t.getAge()).max().orElse(0);
